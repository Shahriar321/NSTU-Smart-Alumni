...
Class Action {
	...
	function delete_user(){
		...
		$delete_full = $this->db->query("DELETE FROM alumnus_bio where id = ".$id);
		if($delete && $delete_full)
			return 1;
	}
	function signup(){
		extract($_POST);
	...
	function update_pass() {
		...
		if(isset($_POST['username']) && isset($_POST['new_password'])) {
			...
			if ($new_password == $confirm_password) {
				...
				// Build an SQL query to update the record with the encrypted password
				...
				echo "<script>alert('New Password and Confirmation do not match. Please try again.');window.location='change_password.php'; </script>";
